  ...
  m.gehs.filter(function(g){return g.name;}).forEach(function(g){
    ...
    if(ps.length===0){
      ...
      ps.forEach(function(p){
        ...
        if(state.fusionMode){
          h+='<div class="prel-checkbox '+(isSelected?'checked':'')+'" onclick="toggleFusionSelect('+p.id+');">Ã¢Å“â€œ</div>';
        }else{
          h+='<div class="prel-status '+(allDone?'done':'pending')+'" onclick="openPrel('+p.id+');">Ã¢Å“â€œ</div>';
        }
...
function renderSmartFusionModal(){
  ...
  state.fusionGroups.forEach(function(group){
    ...
    // Avertissements
    if(hasWarnings){
      h+='<div style="background:#fef3c7;border-radius:6px;padding:6px 8px;font-size:11px;color:#b45309;"><strong>Ã¢Å¡Â Ã¯Ã‚Â¸Ã‚Â Attention:</strong> '+group.warnings.join(', ')+'</div>';
    }
...
function renderTerrainPrel(){
  ...
  if(p.subPrelevements.length>1){
    ...
    for(var t=0;t<p.subPrelevements.length;t++){
      ...
      h+='<button class="tab '+(state.activeSubIndex===t?'active':'')+'" onclick="state.activeSubIndex='+t+';render();">PrÃ©l '+(t+1)+(sb.completed?' Ã¢Å“â€œ':'')+'</button>';
    }
    h+='</div>';
  }
...
function renderSubPrelForm(p,sb,idx){
  ...
  if(!p.agents||p.agents.length===0){
    ...
    p.agents.forEach(function(a){
      ...
      // NÃ‚Â° Pompe avec bouton copier J-1
      h+='<div class="multi-agent-row"><label>NÃ‚Â° Pompe';
  ...
  pl.forEach(function(x,pi){
    ...
    if(pl.length>1)h+='<button class="plage-delete" onclick="removePlage('+p.id+','+idx+','+pi+');">Ã¢Å“â€¢</button>';
    h+='</div>';
  });
  ...
  h+='<button class="btn btn-success" onclick="toggleSubComplete('+p.id+','+idx+');">'+(sb.completed?'Ã¢Å“â€œ ComplÃ©tÃ© - Modifier':'Ã¢Å“â€œ Valider')+'</button></div>';
  return h;
}
...
console.log('Ã¢Å“â€œ Terrain chargÃ©');
